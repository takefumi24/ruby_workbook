# メソッドの定義
  ...
  "コードは基本上から読んでいきます。ただし最初は「def〜end」で定義しているところ以外から読みます。"

  # (2)メソッドを実行することで、変数posts, a_postsにどのような値が代入されるのかがわからない。
  "引数としてpostsを渡して、その戻り値をpostsに再代入している。渡されたpostsをa_postsという変数にコピーして、メソッド内で使用している。"

  # (3) post_reviewメソッドが何をしているのかがわからない
  "今までのレビュー全件が入ったpostsという配列が引数で渡されてpost_reviewが呼び出される。
  postというハッシュにジャンル、タイトル、感想が入った最新の情報を格納される。
  最後に配列に追加して、結果を戻り値としている"
